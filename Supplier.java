...
public class Supplier {
		...
		try {
			...
			try {
				...
				if(isnotEmpty) {
					...
					stmt.close();
					return supplier_id;//logged in successfully
				}
	...
	public int supplier_register(Connection con){ 
		...
		try {
			Statement stmt=con.createStatement(); 
			try {
				String query1="INSERT into supplier (`supplier_id`,`login_password`,`supplier_id`,`pin_code`,`contact_number`,`country_code`)";
		...
		try {
			...
			try {
				...
				else if(flag==3) //Update and delete
				{
					...
					ResultSet rs=stmt.executeQuery("select product_name,product_price,product_rating from products where supplier_id=" +  supplier );
					while(rs.next()) {
						System.out.println();
